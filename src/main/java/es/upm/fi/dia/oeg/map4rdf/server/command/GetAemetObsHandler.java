 ...
 */
public class GetAemetObsHandler implements ActionHandler<GetGeoResource, SingletonResult<GeoResource>> {

	private final Map4rdfDao dao;

	@Inject
	public GetAemetObsHandler(Map4rdfDao dao) {
		this.dao = dao;
	}
	...
	public SingletonResult<GeoResource> execute(GetGeoResource action, ExecutionContext context) throws ActionException {
		...
		try {
			GeoResource resource = dao.getDatosObservacion(uri);
